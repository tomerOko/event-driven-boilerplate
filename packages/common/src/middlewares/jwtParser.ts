...
import { TokenPayload, UnderwriterAuthDetails, minimalTokenPayload } from '../logic/auth/typesAndConsts';
import * as underwriterShared from '../logic/underwriter/shared';
import { user as User } from '../logic/user/model';
import * as userShared from '../logic/user/shared';
import { setAuthDetails, setUnderwriterAuthDetails } from '../utils/auth';
import { httpHeaders } from '../utils/consts/headers';
import { shouldBeHandled } from '../errors/utils';
import { functionWrapper, functionWrapperNoSync } from '../utils/functionWrapper';
import { buildTokenPayloadByUser, createAccessToken, createTokens, verifyAndExtractAccessToken } from '../utils/jwt';
...
export const triggerTokensUpdate = async (user: User, req: Request, res: Response): Promise<void> => {
  return functionWrapper(async () => {
    ...
    res.setHeader(httpHeaders.accessToken, tokens.accessToken);
    res.setHeader(httpHeaders.refreshToken, tokens.refreshToken);
...
export const authHeaderParser = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  return functionWrapper(async () => {
    try {
      ...
      const accessToken = req.headers[httpHeaders.accessToken] as string;

      /** for permitted api consumers */
      const apiSecretKey = req.headers[httpHeaders.secretKey];
...
const validateUser = async (userId: DocumentId | undefined) => {
  return functionWrapper(async () => {
